...
public class ItemsApiService extends RestClientService{
    ...
    public ItemDTO getItemInfoUSD(String itemId) throws ParseException, RestException{
        String currentItemUrl = itemsApiBaseUrl + itemId;
        Response resp = itemsApiClient.get(currentItemUrl);
        ItemDTO item = resp.getData(ItemDTO.class);
        String currentConver = itemsApiBaseUrl + "search?from=" + item.getCurrencyId() +"&to=USD";
        resp = itemsApiClient.get(currentConver);
        resp.getData(ItemDTO.class);
        return item;
    }
}
