...
class UserServiceTest {
	...
	class T0002 {
		...
		void verifyUserToUnfollowExistsOkTest() {
			...
			when(repository.findUserInList(userFollower.getFollowed(), userToUnfollow.getUserId())).thenReturn(
							Optional.of(userToUnfollow));
			when(repository.findUserInList(userToUnfollow.getFollowers(), userFollower.getUserId())).thenReturn(
							Optional.of(userFollower));
			
			//ACT, ASSERT
			
			Assertions.assertDoesNotThrow(() -> service.unfollowUser(userFollower.getUserId(),
			                                                         userToUnfollow.getUserId()));
		...
		void verifyUserToUnfollowExistsExceptionTest() {
			...
			Assertions.assertThrows(BadRequestException.class,
			                        () -> service.unfollowUser(userFollower.getUserId(),
			                                                   userToUnfollow.getUserId()));
	...
	class T0003 {
		...
		void getFollowedListNotOk() {
			...
			when(repository.findUserById(user.getUserId())).thenReturn(Optional.of(user));
			Assertions.assertThrows(BadRequestException.class,
			                        () -> service.getFollowedList(user.getUserId(), order));
		}
	}
