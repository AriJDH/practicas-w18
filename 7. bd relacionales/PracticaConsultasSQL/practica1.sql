...
SELECT m.title, m.rating, m.awards FROM movies_db.movies m WHERE m.rating > 7.5 AND m.awards > 2;
...
SELECT m.title FROM movies_db.movies m WHERE m.rating > 3 AND m.awards >=1 AND m.release_date >= '1989-01-01' AND m.release_date < '2009-01-01';
SELECT m.title, m.release_date FROM movies_db.movies m WHERE m.rating > 3 AND m.awards >=1 AND m.release_date >= '1989-01-01' AND m.release_date < '2009-01-01' ORDER BY m.release_date