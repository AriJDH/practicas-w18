...
public class UserServiceTestConMock {
    ...
    void createSuccess() {
        ...
        User user = new User("Prueba", "prueba", 22);

        when(userRepository.guardarUsuario(user)).thenReturn(user);

        UserDto userDto = new UserDto("Prueba", "prueba", 22);

        UserDto userCreadoDto = userService.crearUsuario(userDto);

        assertNotNull(userCreadoDto);
        assertEquals(userCreadoDto, userDto);
        verify(userRepository).guardarUsuario(user);
    }
}
