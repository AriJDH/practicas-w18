...
import com.socialmeli.be_java_hisp_w18g05.exceptions.NotPromoException;
...
public class ServiceImp implements IService {
    ...
    //For US 0010
    @Override
    public void newPost(NewPromoPostDTORequest promoPost) {
        Integer user_id = promoPost.getUser_id(); // Get id asociated with this post
        Seller seller = repository.getByIdSeller(user_id); // Get seller from repository
        ...
        //Create DTO of the product
        Product product = new Product(promoPost.getProduct().getProduct_id(), promoPost.getProduct().getProduct_name(), promoPost.getProduct().getType(), promoPost.getProduct().getBrand(), promoPost.getProduct().getColor(), promoPost.getProduct().getNotes());
    ...
    //For US 0011
    @Override
    public SellerPromoPostCountDTOResponse promosCount(Integer user_id) {
        Seller seller = repository.getByIdSeller(user_id); // Get seller from repository
        if (seller == null) {
            throw new NotFoundException("Seller id " + user_id + " not found"); // Throw exception if seller doesn't exist
        }
