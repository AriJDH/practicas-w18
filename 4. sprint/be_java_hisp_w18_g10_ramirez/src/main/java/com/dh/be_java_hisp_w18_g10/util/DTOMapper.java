...
public class DTOMapper {
    ...
    public static Post mapToPost(ProductOnPromotionDTOreq productOnPromotion){
        Post post = new Post();
        post.setUserId(productOnPromotion.getUser_id());
        post.setDate(DateHandler.StringToDate(productOnPromotion.getDate()));
        Product product = getInstance().map(productOnPromotion.getProduct(), Product.class);
        product.setProductId(productOnPromotion.getProduct().getProduct_id());
        product.setProductName(productOnPromotion.getProduct().getProduct_name());
        post.setProduct(product);
        post.setCategory(productOnPromotion.getCategory());
        post.setPrice(productOnPromotion.getPrice());
        post.setHasPromo(productOnPromotion.isHas_promo());
        post.setDiscount(productOnPromotion.getDiscount());
        return post;
    }
    ...
    public static PromoPostDTOres mapToPromoPost(Post post){
        PromoPostDTOres postRes = new PromoPostDTOres();
        postRes.setUser_id(post.getUserId());
        postRes.setPost_id(post.getPost_id());
        //Parse fecha, ya que el DTO la maneja como String
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
        ...
        //Fin mapeo
        postRes.setCategory(post.getCategory());
        postRes.setHasPromo(post.isHasPromo());
