...
public class UI {
    ...
    private PrototipoDos prototypeOne;
    private PrototipoTres prototypeTwo;

    public UI() {
    }

    public UI(int serieType, PrototipoDos PrototipoDos, PrototipoTres PrototipoTres) {
        this.serieType = serieType;
        this.PrototipoDos = PrototipoDos;
        this.PrototipoTres = PrototipoTres;
    }
    ...
    private void firstValue() {
        if (this.serieType == 1) {
            ...
            PrototipoDos.setInitialValue(newValue1);
        } else if (this.serieType == 2) {
            Integer newValue2 = input.nextInt();
            PrototipoTres.setInitialValue(newValue2);
        }
    }
    ...
    private void sumValue() {
        ...
        if (this.serieType == 1) {
            ...
            PrototipoDos.setA(a1);
        } else if (this.serieType == 2) {
            Integer a2 = input.nextInt();
            PrototipoTres.setA(a2);
        }
    ...
    private void showNextValue() {
        if (this.serieType == 1) {
            System.out.println(
                    "------------------------------------------------ \n" +
                            " El próximo número es: " + PrototipoDos.nextValue(PrototipoDos.getA()) +
                            " \n------------------------------------------------ ");
        } else if (this.serieType == 2) {
            System.out.println(
                    "------------------------------------------------ \n" +
                            " El próximo número es: " + PrototipoTres.nextValue(PrototipoTres.getA()) +
                            " \n------------------------------------------------ ");
        }
    }
    /* ---------------- Opción 4 ----------------------- */
    private void resetValue() {
        if (this.serieType == 1) {
            PrototipoDos.setValue();
        } else if (this.serieType == 2) {
            PrototipoTres.setValue();
        }
    }
}
